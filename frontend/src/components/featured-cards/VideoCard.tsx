...
export function VideoCard({ item }: VideoCardProps) {
  return (
    <Link href={`/learning/video/${item.content.id}`}>
      <Card className="overflow-hidden h-[300px] border-none shadow-md bg-gradient-to-r from-[#2c5530] to-[#1a3a1a] text-white">
        <div className="grid h-full md:grid-cols-[1fr_1fr]">
          <div className="relative aspect-video md:aspect-auto">
            <img
              src={item.content.thumbnail_url || "/placeholder.svg?height=400&width=600"}
            ...
            <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
              <Button
                variant="outline"
                className="rounded-full h-16 w-16 p-0 border-2 border-white bg-black/30 hover:bg-black/50"
              >
                <Play className="h-8 w-8 text-white" />
              </Button>
            </div>
            <div className="absolute bottom-4 right-4 bg-black/60 text-white text-sm px-3 py-1 rounded-md backdrop-blur-sm">
              {item.content.duration || '10:00'}
            </div>
          </div>
