...
import { ArrowLeft, BookOpen, X, Save, Trash } from 'lucide-react';
...
export default function EditWikiPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        router.push(`/learning/wiki/${materialId}`);
      } else {
        const errorData = await response.json();
        console.error('Error response:', errorData);
        setError(errorData.error || 'Failed to update wiki article. Please try again.');
      }
    } catch (error) {
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <Alert variant="destructive" className="max-w-md">
          <AlertDescription>
            {error || 'Wiki article not found. Redirecting...'}
          </AlertDescription>
        </Alert>
      </div>
    );
          ...
          <Card className="border-[#d1e0d3] overflow-hidden bg-white shadow-md">
            <CardHeader className="bg-[#f0f4e9] border-b border-[#d1e0d3] pb-6">
              <div className="flex items-center mb-2">
                ...
                <h1 className="text-2xl font-bold text-[#2c5530]">Edit Wiki Article</h1>
              </div>
              <p className="text-[#5a7d61]">Update your wiki entry with the latest information.</p>
            </CardHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="pt-4 flex justify-end space-x-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => router.push(`/learning/wiki/${params.id}`)}
